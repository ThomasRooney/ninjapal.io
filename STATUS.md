...
### Known small bugs
- ~~Cold-load hydration race~~ ✅ **fixed 2026-06-12** by the TanStack Start 1.121→1.168 upgrade (10/10 cold loads vs ~70-80% before). A cosmetic React #418 hydration-mismatch warning still logs on every load but recovery always succeeds — chase the markup mismatch someday. ⚠️ Deploy pipeline note: Start 1.168 removed nitro; `@tanstack/nitro-v2-vite-plugin` (vite config) restores the vercel preset that emits `.vercel/output`. Without it the build ships only `dist/` and a git deploy 404s the whole site (happened once; rolled back via `vercel rollback <dpl>` in seconds).
