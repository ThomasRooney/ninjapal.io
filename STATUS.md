...
| Email | inbound: ForwardEmail catch-all via DNS (`*@pitminder.com` → Thomas's gmail); outbound: Resend (templates in `src/emails`) | ✅ fully working (2026-06-11): pitminder.com verified in Resend (eu-west-1, DKIM/SPF/DMARC in Vercel DNS), send-only key in Vercel + local .env, `EMAIL_FROM=PitMinder <no-reply@pitminder.com>`; round-trip (magic link + direct send → forwarding) verified |
