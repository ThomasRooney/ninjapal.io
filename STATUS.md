...
| Email | inbound: ForwardEmail catch-all via DNS (`*@pitminder.com` → Thomas's gmail); outbound: Resend (templates in `src/emails`) | **blocked: RESEND_API_KEY in .env is invalid** — create a fresh key, update local .env + Vercel envs, then verify the pitminder.com domain in Resend (API: POST /domains) and add its DKIM/SPF records to Vercel DNS. `EMAIL_FROM=PitMinder <no-reply@pitminder.com>` already set |
| Login methods | email+password, magic link, Google OAuth | magic-link + verification emails no-op until Resend key fixed; Google needs `https://app.pitminder.com/api/auth/callback/google` + `http://localhost:5173/api/auth/callback/google` added to the OAuth client's authorized redirect URIs in Google Cloud Console |
