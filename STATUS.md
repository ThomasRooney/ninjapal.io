...
| Email | inbound: ForwardEmail catch-all via DNS (`*@pitminder.com` → Thomas's gmail); outbound: Resend (templates in `src/emails`) | **outbound ✅ live** (2026-06-11): pitminder.com verified in Resend (eu-west-1, DKIM/SPF/DMARC in Vercel DNS), send-only key in Vercel + local .env, `EMAIL_FROM=PitMinder <no-reply@pitminder.com>` — magic links/verification mails to real user addresses work. **Inbound ❌ bouncing**: ForwardEmail's free tier blocks domains registered <90 days ago, so `hello@pitminder.com` (the legal-pages contact) bounces. Options pending Thomas's pick: ForwardEmail $3/mo (DNS already set), ImprovMX free (needs account + MX swap), or revert contact to gmail |
