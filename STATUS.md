...
4. **The AI pitmaster** (vision set 2026-06-12, demoed on the marketing site as mocks) — the product north star: PitMinder *runs the pit*. Building blocks, roughly in order:
   - **Device control v1**: `sendCookCommand` via the Railway worker POSTing Ayla datapoints (`SET_Cook_Command`/`SET_GrillPower`); manual setpoint/mode/smoke UI; e2e with mocked Ayla. (Sync already lives on the worker — control belongs there too, not on Vercel.)
   - **Auto hold-warm**: when a probe hits its doneness target, automatically drop the setpoint (~65°C) and notify "we lowered the temperature — it's ready". Multi-probe: per-probe targets and staggered hold decisions.
   - **Coaching alerts**: actionable pushes ("spritz now", "wrap or ride?", "ribs on in 5") driven by cook-analysis state transitions; delivered via push + email (infra exists).
   - **Sentence-to-plan**: "eat at 19:00, 4kg brisket + 2 racks" → LLM-generated cook plan (timeline with ack/steer), continuously replanned against live telemetry. This is the LLM integration point — plan schema + claude API + a plan-execution loop in the worker.
